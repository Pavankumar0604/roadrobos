...
import { CogIcon, LogoutIcon, PencilAltIcon, UsersIcon, ViewGridIcon, XIcon, MenuAlt2Icon, ClipboardListIcon, LocationMarkerIcon, TagIcon, MailIcon, TrashIcon, PlusIcon, StarIcon, CreditCardIcon, DocumentCheckIcon, DocumentDownloadIcon } from './icons/Icons';
...
type AdminPanel = 'dashboard' | 'fleet' | 'locations' | 'offers' | 'enquiries' | 'reviews' | 'transactions' | 'content' | 'users' | 'settings' | 'employees' | 'applications';
...
const AdminDashboard: React.FC<AdminDashboardProps> = (props) => {
  ...
  const [activePanel, setActivePanel] = useState<AdminPanel>('applications');
  ...
  const renderPanel = () => {
    switch (activePanel) {
      ...
      case 'dashboard':
      default:
        return <DashboardHomePanel user={user} />;
    }
  };
  ...
  const SidebarContent = () => (
    <>
      ...
      <nav className="flex-grow p-2 overflow-y-auto">
          {navItems.map(item => (
              <button
                  ...
                  className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-md text-sm font-medium transition-colors ${activePanel === item.id ? 'bg-white/20' : 'hover:bg-white/10'}`}
              >
                  {item.icon}
      ...
      <div className="p-4 border-t border-white/20">
          ...
          <p className="text-xs opacity-70">{user.role.name}</p>
          <button onClick={onLogout} className="w-full flex items-center justify-center gap-2 mt-4 bg-white/10 hover:bg-white/20 text-white font-semibold py-2 rounded-lg text-sm">
    ...
    <div className="flex min-h-screen bg-accent">
      ...
      {/* Mobile Sidebar */}
       <div className={`fixed inset-0 z-40 lg:hidden ${isSidebarOpen ? 'block' : 'hidden'}`}>
        ...
        <aside className={`fixed top-0 left-0 h-full w-64 bg-primary text-white flex flex-col transition-transform duration-300 ease-in-out ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
          <SidebarContent />
        </aside>
      </div>
...
const DashboardHomePanel: React.FC<{ user: AdminUser }> = ({ user }) => (
    <div>
        <h1 className="text-3xl font-bold">Welcome back, {user.name}!</h1>
        <p className="text-gray-600 mt-1">Here's a quick overview of your site.</p>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-8">
            <Card className="p-6 text-center"><h3 className="text-4xl font-bold">1,250</h3><p className="text-gray-500">Total Bookings (Month)</p></Card>
            <Card className="p-6 text-center"><h3 className="text-4xl font-bold">85%</h3><p className="text-gray-500">Fleet Utilization</p></Card>
            <Card className="p-6 text-center"><h3 className="text-4xl font-bold">58</h3><p className="text-gray-500">New Partners</p></Card>
        </div>
        <div className="mt-8">
            <Card className="p-6">
                <h2 className="text-xl font-bold mb-4">Quick Actions</h2>
                <div className="flex flex-wrap gap-4">
                    <button className="bg-secondary text-white font-semibold py-2 px-4 rounded-lg">Add New Bike</button>
                    <button className="border border-primary text-primary font-semibold py-2 px-4 rounded-lg">Manage Offers</button>
                    <button className="border border-primary text-primary font-semibold py-2 px-4 rounded-lg">View Reports</button>
                </div>
            </Card>
        </div>
    </div>
...
const BikeManagementPanel: React.FC<{ fleet: Bike[]; setFleet: React.Dispatch<React.SetStateAction<Bike[]>> }> = ({ fleet, setFleet }) => {
    ...
    const handleSave = (bikeData: Bike) => {
        if (bikeData.id) {
            ...
            setFleet(prev => [...prev, { ...bikeData, id: Date.now() }]);
        }
...
const LocationManagementPanel: React.FC<{ locations: string[]; setLocations: React.Dispatch<React.SetStateAction<string[]>> }> = ({ locations, setLocations }) => {
    ...
    const handleSave = (newName: string, originalName?: string) => {
        if (originalName) {
...
const OfferManagementPanel: React.FC<{ offers: Offer[]; setOffers: React.Dispatch<React.SetStateAction<Offer[]>> }> = ({ offers, setOffers }) => {
    ...
    const handleSave = (offer: Offer) => {
        if (offer.id) {
            ...
            setOffers(prev => [...prev, { ...offer, id: `o${Date.now()}`, totalUses: 0, descriptionBullets: [], imagePlaceholder: 'https://picsum.photos/seed/new-offer/600/400', applicableCities: ['All'] }]);
        }
        setIsModalOpen(false);
    };
    const handleToggleStatus = (id: string) => {
        setOffers(prev => prev.map(o => o.id === id ? { ...o, status: o.status === 'Active' ? 'Disabled' : 'Active' } : o));
    };
...
const TransactionManagementPanel: React.FC<{ transactions: Transaction[]; setTransactions: React.Dispatch<React.SetStateAction<Transaction[]>> }> = ({ transactions, setTransactions }) => {
    const handleExport = () => {
        const headers = ["Transaction ID", "Booking ID", "Customer Name", "Amount", "Status", "Date"];
        const csvContent = "data:text/csv;charset=utf-8," 
            + headers.join(",") + "\n" 
            + transactions.map(t => [t.id, t.bookingId, t.customerName, t.amount, t.status, new Date(t.date).toLocaleString()].join(",")).join("\n");
    ...
    return (
        <div>
            <div className="flex justify-between items-center mb-6">
                ...
                <button onClick={handleExport} className="border border-primary text-primary font-semibold py-2 px-4 rounded-lg flex items-center gap-2">
                    Export to Excel (.csv)
                </button>
            </div>
                            ...
                            <tbody className="bg-white divide-y divide-card">
                                {transactions.map(t => (
                                    <tr key={t.id}>
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap text-text-body">₹{t.amount.toFixed(2)}</td>
                                        <td className="px-6 py-4 whitespace-nowrap">
                                            <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${t.status === 'Paid' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'}`}>
                                                {t.status}
                                            </span>
...
const UserManagementPanel: React.FC<{ adminUsers: AdminUser[]; setAdminUsers: React.Dispatch<React.SetStateAction<AdminUser[]>> }> = ({ adminUsers, setAdminUsers }) => {
    ...
    const handleSave = (user: AdminUser) => {
        if (user.id) {
            setAdminUsers(prev => prev.map(u => u.id === user.id ? { ...u, ...user, passwordHash: user.passwordHash || u.passwordHash } : u));
        } else {
            setAdminUsers(prev => [...prev, { ...user, id: `u${Date.now()}` }]);
        }
        setIsModalOpen(false);
    };

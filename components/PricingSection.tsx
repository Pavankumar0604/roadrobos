...
type TariffType = 'Hourly' | 'Daily' | 'Weekly' | 'Monthly' | 'Quarterly' | 'Yearly';
const TARIFFS: TariffType[] = ['Hourly', 'Daily', 'Weekly', 'Monthly', 'Quarterly', 'Yearly'];
...
const BIKE_TYPES: ('All' | Bike['type'])[] = ['All', 'Scooter', 'Gear', 'Electric', 'Superbike'];
...
const PricingSection: React.FC<PricingSectionProps> = ({ city, onSelectBike, bikes }) => {
    ...
    const getTariffDetails = (bike: Bike) => {
        ...
        if (activeTariff === 'Quarterly') priceUnit = '/qtr';
        if (activeTariff === 'Yearly') priceUnit = '/yr';

        let kmLimit: string | number = "Unlimited";
        let kmUnit = "";

...
const BookingWidget: React.FC<{ bike: Bike, searchParams: SearchParams | null, onBookNow: BikeDetailPageProps['onBookNow'] }> = ({ bike, searchParams, onBookNow }) => {
    ...
    if (isComingSoon) {
        ...
        disableReason = 'This bike will be available for booking shortly.';
    } else if (!isBookable) {
        buttonLabel = 'Unavailable';
        disableReason = `This bike is currently ${bike.availability?.toLowerCase() || 'unavailable'}.`;
    }
        ...
        <Card className="p-6 sticky top-24">
            ...
            <div className="mt-6 pt-4 border-t">
                ...
                <button
                    ...
                    disabled={!isBookable || !!error || totalPrice === 0}
                    className="w-full mt-4 bg-secondary text-white font-bold py-3 rounded-lg hover:bg-opacity-90 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                >
                    {buttonLabel}
                </button>
                {!isBookable && (

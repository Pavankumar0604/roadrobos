...
-- Ensure missing bike columns exist
DO $$ 
BEGIN 
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='bikes' AND column_name='service_status') THEN
        ALTER TABLE public.bikes ADD COLUMN service_status TEXT DEFAULT 'none' CHECK (service_status IN ('none', 'pending', 'in_progress', 'completed'));
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='bikes' AND column_name='current_status') THEN
        ALTER TABLE public.bikes ADD COLUMN current_status TEXT DEFAULT 'readyRent';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='bikes' AND column_name='checks') THEN
        ALTER TABLE public.bikes ADD COLUMN checks JSONB DEFAULT '{}'::jsonb;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='bikes' AND column_name='assigned_tech') THEN
        ALTER TABLE public.bikes ADD COLUMN assigned_tech TEXT;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='bikes' AND column_name='spare_parts') THEN
        ALTER TABLE public.bikes ADD COLUMN spare_parts JSONB DEFAULT '[]'::jsonb;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='bikes' AND column_name='parts_report') THEN
        ALTER TABLE public.bikes ADD COLUMN parts_report TEXT;
    END IF;
END $$;
...
-- Ensure missing rider columns exist
DO $$ 
BEGIN 
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='rider_information' AND column_name='delivery_executive') THEN
        ALTER TABLE public.rider_information ADD COLUMN delivery_executive BOOLEAN DEFAULT false;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='rider_information' AND column_name='delivery_id') THEN
        ALTER TABLE public.rider_information ADD COLUMN delivery_id TEXT;
    END IF;
END $$;
...
-- Ensure missing transaction columns exist
DO $$ 
BEGIN 
...
DO $$ 
BEGIN 
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='offers' AND column_name='auto_applied') THEN
        ALTER TABLE public.offers ADD COLUMN auto_applied BOOLEAN DEFAULT false;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='offers' AND column_name='ends_in') THEN
        ALTER TABLE public.offers ADD COLUMN ends_in INTEGER;
    END IF;
END $$;
...
-- Ensure missing city column exists
DO $$ 
BEGIN 
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='enquiries' AND column_name='city') THEN
        ALTER TABLE public.enquiries ADD COLUMN city TEXT;
    END IF;
END $$;
...
-- Handle renamed columns efficiently
DO $$ 
BEGIN 
    -- Rename 'reviewer' to 'name' if it exists
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='reviews' AND column_name='reviewer') THEN
        ALTER TABLE public.reviews RENAME COLUMN reviewer TO name;
    ELSIF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='reviews' AND column_name='name') THEN
        ALTER TABLE public.reviews ADD COLUMN name TEXT NOT NULL DEFAULT 'Anonymous';
    END IF;

    -- Rename 'comment' to 'text' if it exists
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='reviews' AND column_name='comment') THEN
        ALTER TABLE public.reviews RENAME COLUMN comment TO text;
    ELSIF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='reviews' AND column_name='text') THEN
...
DO $$ 
BEGIN 
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='pickup_locations' AND column_name='status') THEN
        ALTER TABLE public.pickup_locations ADD COLUMN status TEXT DEFAULT 'active' CHECK (status IN ('active','busy','unavailable'));
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='pickup_locations' AND column_name='is_active') THEN
        ALTER TABLE public.pickup_locations ADD COLUMN is_active BOOLEAN DEFAULT true;
    END IF;
END $$;
...
-- Ensure missing employee columns exist
DO $$ 
BEGIN 
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='employees' AND column_name='specialty') THEN
        ALTER TABLE public.employees ADD COLUMN specialty TEXT DEFAULT 'General';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='employees' AND column_name='tasks') THEN
        ALTER TABLE public.employees ADD COLUMN tasks INTEGER DEFAULT 0;
    END IF;
    ...
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='employees' AND column_name='joined_at') THEN
        ALTER TABLE public.employees ADD COLUMN joined_at TEXT;
    END IF;
END $$;
